    ...
    <div v-if="developer">
      <header class="pb-8 border-b border-border mb-8">
        ...
        <div class="flex flex-wrap justify-center md:justify-start gap-4">
          <a v-if="developer.linkedinUrl" :href="developer.linkedinUrl" target="_blank" rel="noopener" class="flex items-center gap-2 px-5 py-3 bg-background-card border border-border rounded-full text-foreground no-underline text-sm transition-all hover:bg-background-card-hover hover:border-foreground-muted">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
              <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
            </svg>
            LinkedIn
          </a>
          <a v-if="developer.githubUrl" :href="developer.githubUrl" target="_blank" rel="noopener" class="flex items-center gap-2 px-5 py-3 bg-background-card border border-border rounded-full text-foreground no-underline text-sm transition-all hover:bg-background-card-hover hover:border-foreground-muted">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
          ...
          <a v-if="developer.website" :href="developer.website" target="_blank" rel="noopener" class="flex items-center gap-2 px-5 py-3 bg-background-card border border-border rounded-full text-foreground no-underline text-sm transition-all hover:bg-background-card-hover hover:border-foreground-muted">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <section v-if="developer.skills?.length" class="py-8 border-b border-border">
        ...
        <div class="flex flex-wrap gap-3">
          <span v-for="skill in developer.skills" :key="skill" class="px-5 py-2 bg-background-card border border-border rounded-full text-sm">
            {{ skill }}
          </span>
        </div>
      </section>
      ...
      <section v-if="developer.speakerProfile" class="py-8">
        <div class="inline-flex items-center gap-3 px-6 py-3 bg-background-card border border-border rounded-full text-sm font-medium mb-6">
        ...
        <div v-if="developer.speakerProfile.topics?.length" class="flex flex-wrap items-center gap-3 mb-4">
          ...
          <span v-for="topic in developer.speakerProfile.topics" :key="topic" class="px-4 py-2 bg-background-card border border-border rounded-full text-sm">
            {{ topic }}
          </span>
        </div>
