  ...
  <div class="max-w-7xl mx-auto px-4 md:px-16">
    ...
    <section class="py-8 border-b border-border">
      <div class="flex flex-col md:flex-row gap-6 items-stretch md:items-end">
        ...
        <button v-if="filters.location || filters.verified" @click="clearFilters" class="px-6 py-3 bg-transparent border border-border rounded-lg text-foreground-muted text-sm cursor-pointer transition-all hover:border-foreground hover:text-foreground">
          Effacer
        </button>
      </div>
    </section>
    ...
    <section class="py-8 border-b border-border">
      <div class="flex items-center gap-4 px-6 py-4 bg-background-card border border-border rounded-xl max-w-lg">
    ...
    <section class="py-12">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="company in companies" :key="company.id" class="flex flex-col gap-4 p-6 bg-background-card border border-border rounded-2xl">
    ...
    <div v-if="showAddModal" class="fixed inset-0 bg-black/80 flex items-center justify-center z-[1000] p-8" @click.self="showAddModal = false">
      <div class="bg-background border border-border rounded-2xl p-8 w-full max-w-lg">
        ...
        <div class="mb-6">
          ...
          <input v-model="newCompany.name" type="text" class="w-full px-4 py-3 bg-background-card border border-border rounded-lg text-foreground text-sm focus:outline-none focus:border-foreground-muted" placeholder="Nom de l'entreprise" />
        </div>

        <div class="mb-6">
          <label class="block text-xs uppercase tracking-widest text-foreground-muted mb-2">Site web</label>
          <input v-model="newCompany.website" type="url" class="w-full px-4 py-3 bg-background-card border border-border rounded-lg text-foreground text-sm focus:outline-none focus:border-foreground-muted" placeholder="https://..." />
        </div>

        <div class="mb-6">
          <label class="block text-xs uppercase tracking-widest text-foreground-muted mb-2">Ville</label>
          <input v-model="newCompany.location" type="text" class="w-full px-4 py-3 bg-background-card border border-border rounded-lg text-foreground text-sm focus:outline-none focus:border-foreground-muted" placeholder="Paris" />
        </div>

        <div class="mb-6">
          <label class="block text-xs uppercase tracking-widest text-foreground-muted mb-2">Description</label>
          <textarea v-model="newCompany.description" class="w-full px-4 py-3 bg-background-card border border-border rounded-lg text-foreground text-sm focus:outline-none focus:border-foreground-muted min-h-[100px] resize-y" placeholder="Description de l'entreprise"></textarea>
        </div>
    ...
    <div v-if="showReviewModal" class="fixed inset-0 bg-black/80 flex items-center justify-center z-[1000] p-8" @click.self="showReviewModal = false">
      <div class="bg-background border border-border rounded-2xl p-8 w-full max-w-lg">
        ...
        <div class="mb-6">
          ...
          <div class="flex gap-2">
            <button
              ...
              :class="['flex-1 py-3 px-4 rounded-lg text-sm cursor-pointer transition-all', newReview.isInclusive ? 'bg-foreground text-background border-foreground' : 'bg-background-card text-foreground-muted border border-border']"
            >
              Oui
            </button>
            <button
        ...
        <div class="mb-6">
          ...
          <textarea v-model="newReview.comment" class="w-full px-4 py-3 bg-background-card border border-border rounded-lg text-foreground text-sm focus:outline-none focus:border-foreground-muted min-h-[100px] resize-y" placeholder="Votre expérience..."></textarea>
        </div>
